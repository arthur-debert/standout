...
//!     Complete(CompleteArgs),  // → handlers::complete
//! }
//! ```
//!
//! # Features
...
//! ## Explicit Overrides
//!
//! Override any convention with variant attributes:
//!
//! ```rust,ignore
//! #[derive(Subcommand, Dispatch)]
//! #[dispatch(handlers = handlers)]
...
//!
//!     #[dispatch(
//!         handler = handlers::mark_done,
//!         template = "complete.j2",
//!         pre_dispatch = validate_ids,
...
//!     Backup(BackupArgs),    // → handlers::db::backup
//! }
//! ```
//!
...
//! # Generated Code
//!
//! The macro generates a `dispatch_config` method:
//!
//! ```rust,ignore
//! impl Commands {
//!     pub fn dispatch_config() -> impl FnOnce(GroupBuilder) -> GroupBuilder {
//!         |builder| {
//!             builder
//!                 .command("add", handlers::add)
...
pub fn dispatch_derive_impl(input: DeriveInput) -> Result<TokenStream> {
    ...
    let expanded = quote! {
        impl #enum_name {
            /// Returns a dispatch configuration closure for use with `Outstanding::builder().commands()`.
            ///
            /// Generated by `#[derive(Dispatch)]`.
            pub fn dispatch_config() -> impl FnOnce(::outstanding_clap::GroupBuilder) -> ::outstanding_clap::GroupBuilder {
                |__builder: ::outstanding_clap::GroupBuilder| {
