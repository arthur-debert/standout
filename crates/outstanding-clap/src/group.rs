...
use outstanding::{render_or_serialize_with_context, Theme};
use serde::Serialize;
use std::collections::HashMap;
use std::sync::Arc;

use crate::dispatch::{DispatchFn, DispatchOutput};
use crate::handler::{CommandContext, CommandResult, FnHandler, Handler};
use crate::hooks::Hooks;
use crate::outstanding::get_terminal_width;
...
impl<H> CommandConfig<H> {
    ...
    where
        F: Fn(&ArgMatches, &CommandContext) -> Result<(), crate::hooks::HookError>
            + Send
            + Sync
            + 'static,
    {
    ...
    where
        F: Fn(
                ...
                &CommandContext,
                serde_json::Value,
            ) -> Result<serde_json::Value, crate::hooks::HookError>
            + Send
            + Sync
            + 'static,
    {
    ...
    where
        F: Fn(
                ...
                &CommandContext,
                crate::hooks::Output,
            ) -> Result<crate::hooks::Output, crate::hooks::HookError>
            + Send
            + Sync
            + 'static,
    {
