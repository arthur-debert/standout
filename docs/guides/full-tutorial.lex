...
This is a small, focused guide for adopting Outstanding in a working shell application.
Each step is self-sufficient, takes a positive step towards a sane CLI design, and can be incrementally merged.
This can be done for one command (probably a good idea), then replicated to as many as you'd like.

Note that only 2 out of 8 steps are Outstanding related. The others are generally good practices and
clear designs for maintainable shell programs. This is not an accident, as Outstanding's goal is to allow
your app to keep a great structure effortlessly, while providing testability, rich and fast output design,
and more.
...
2. Hard Split Logic and Formatting
  ...
  Now, your command should be split into two functions: the logic handler and its rendering. Don't worry
  about the specifics, do the straightest path from your current code.

  This is the one key step, the key design rule. And that's not because Outstanding requires it,
  rather the other way around: Outstanding is designed on top of it, and keeping it separate and easy to
